/* ── Nav scroll behavior ── */
  ...
  function closeMobileMenu() {
    ...
    if (mobilePortfolioTrigger) {
      mobilePortfolioTrigger.setAttribute('aria-expanded', 'false');
      mobilePortfolioSubmenu.classList.remove('open');
    }
  }

  /* ── Portfolio accordion (mobile) ── */
  const mobilePortfolioTrigger = document.getElementById('mobile-portfolio-trigger');
  const mobilePortfolioSubmenu = document.getElementById('mobile-portfolio-submenu');
  if (mobilePortfolioTrigger) {
    mobilePortfolioTrigger.addEventListener('click', () => {
      const isOpen = mobilePortfolioSubmenu.classList.toggle('open');
      mobilePortfolioTrigger.setAttribute('aria-expanded', String(isOpen));
    });
  }
